(() => {
  ...
  let last = 0, accumulator = 0, shake = 0, tilt = 0, muted = false, audio, stuckClock = 0, searchPulses = 0;
  const keys = { left: false, right: false };
  const ball = { x: 574, y: 824, vx: 0, vy: 0, r: 11, active: false, gated: false, trail: [] };
  const bumpers = [{x:230,y:310,r:45,color:"#e34f32",hit:0},{x:410,y:280,r:42,color:"#dfc456",hit:0},{x:330,y:445,r:48,color:"#d8ddd0",hit:0}];
  const posts = [{x:172,y:600,r:15},{x:468,y:600,r:15},{x:117,y:720,r:12},{x:523,y:720,r:12}];
  const lanes = [{x:182,lit:false},{x:320,lit:false},{x:458,lit:false}];
  const walls = [
    [55,105,55,690],[55,105,125,48],[125,48,510,48],[510,48,550,95],[550,95,550,790],
    [55,690,120,785],[120,785,180,825],[460,825,520,785],[520,785,550,720],
    [80,570,150,680],[150,680,185,720],[560,570,490,680],[490,680,455,720],
    [78,165,145,115],[495,115,540,180]
  ];
  const targets = [{x:105,y:405,w:9,h:70,hit:0},{x:525,y:405,w:9,h:70,hit:0}];
  const flippers = [
    {x:190,y:790,len:125,angle:.32,rest:.32,active:-.42,dir:1,down:false},
    {x:450,y:790,len:125,angle:Math.PI-.32,rest:Math.PI-.32,active:Math.PI+.42,dir:-1,down:false}
  ];
  ...
  function resetBall(){ball.x=574;ball.y=824;ball.vx=0;ball.vy=0;ball.active=false;ball.gated=false;ball.trail=[];resetSearch();ui.status.textContent="HOLD SPACE TO LAUNCH"}
  ...
  function launch(){if(state!=="playing") return;if(!ball.active){ball.active=true;ball.gated=true;ball.x=475;ball.y=170;ball.vx=-360;ball.vy=210;addScore(500,"SKILL SHOT +500");ui.status.textContent="BALL IN PLAY";sound(120,.16,"sawtooth",.06)}}
  ...
  function physics(dt){
    ...
    if(ball.y>95&&ball.x>551&&ball.x<608){if(ball.x+ball.r>608){ball.x=608-ball.r;ball.vx=-Math.abs(ball.vx)*.7}if(ball.x-ball.r<551){ball.x=551+ball.r;ball.vx=Math.abs(ball.vx)*.7}}
  ...
  function draw(){
    ...
    ctx.fillStyle="#262922";ctx.fillRect(551,95,57,800);ctx.fillStyle="#e34f32";ctx.fillRect(551,95,4,800);
    ...
    ctx.strokeStyle="#dfc456";ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(568,855);ctx.lineTo(592,855);ctx.stroke();ctx.fillStyle="#777c70";ctx.fillRect(574,870,12,55);
  ...
  addEventListener("keydown",e=>{if(["Space","ArrowLeft","ArrowRight"].includes(e.code))e.preventDefault();if(e.repeat)return;if(e.code==="KeyZ"||e.code==="ArrowLeft")setFlipper("left",true);if(e.code==="Slash"||e.code==="ArrowRight")setFlipper("right",true);if(e.code==="Space"){if(state==="ready"||state==="gameover")startGame();else launch()}if(e.code==="KeyP")togglePause();if((e.code==="ArrowLeft"||e.code==="ArrowRight")&&ball.active){ball.vx+=e.code==="ArrowLeft"?-70:70;tilt++;if(tilt>8)callout("EASY, HOTSHOT")}});
  addEventListener("keyup",e=>{if(e.code==="KeyZ"||e.code==="ArrowLeft")setFlipper("left",false);if(e.code==="Slash"||e.code==="ArrowRight")setFlipper("right",false)});
  ...
  bindHold("leftTouch","left");bindHold("rightTouch","right");document.getElementById("launchTouch").addEventListener("pointerdown",()=>{if(state==="ready"||state==="gameover")startGame();else launch()});
